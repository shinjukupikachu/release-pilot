...
_RELEASES = [
    {
        ...
        "internal_announcement": "## NyankoOS v1.0.0 — General Availability\n\nAfter 18 months of development and beta testing with 12 pilot customers, NyankoOS 1.0 is shipping. This is our first production-ready release.\n\n**Core capabilities shipping today:**\n- Pick-and-place task execution for standard 6-DOF arms\n- Vision-based object detection (YOLOv5)\n- REST API for task management\n- Web operator dashboard\n- SQLite-backed task history",
        "customer_notes": "## NyankoOS v1.0.0 — Generally Available\n\nWe're excited to announce the general availability of NyankoOS 1.0, the first production release of Nyanko's robot operating system.\n\n### What's Included\n\n**Core Robot Control**\n- Execute pick-and-place tasks on standard 6-DOF industrial robot arms\n- Real-time trajectory planning with collision avoidance\n- Graceful E-stop and recovery handling\n\n**Vision System**\n- Object detection and pose estimation using onboard cameras\n- Supports structured-light and stereo depth sensors\n- Confidence-based pick decision gating\n\n**Operator Dashboard**\n- Web-based UI for task monitoring and configuration\n- Real-time robot status, joint positions, and camera feeds\n- Task history and basic cycle time metrics\n\n**REST API**\n- Full task lifecycle management via REST\n- Webhook support for task completion events\n- Python SDK included",
        "marketing_notes": "## NyankoOS 1.0: The Industrial Robot OS Built for Production\n\nToday, Nyanko launches NyankoOS 1.0 — the first robot operating system designed from the ground up for production warehouse automation.\n\nUnlike research-grade platforms, NyankoOS 1.0 ships with everything needed to run a live picking cell: vision, planning, control, and a production-ready API. No PhD required.",
        "traceability": [
            {
                "short_hash": "1a2b3c4",
    ...
    },
    {
        ...
        "customer_notes": "## NyankoOS v1.1.0 — What's New\n\n### New Features\n\n**Conveyor Tracking** — Robots can now pick items from moving conveyors at speeds up to 1.5 m/s. Configure belt speed via the dashboard or API.\n\n**Multi-SKU Support** — A single cell can now handle up to 20 distinct SKU types simultaneously with per-SKU grasp profiles.\n\n**Barcode Fallback** — When vision confidence falls below your configured threshold, NyankoOS automatically falls back to barcode scanning to confirm item identity before picking.\n\n**Python SDK on PyPI** — Install with `pip install nyankoos-sdk`. Full API coverage with async support.\n\n### Bug Fixes\n\n- Fixed gripper jaw calibration drift after 10,000 cycles\n- Fixed waypoint editor crash on import of files with more than 500 waypoints\n- Fixed dashboard session timeout with no notification",
    ...
    },
    {
        ...
        "customer_notes": "## NyankoOS v1.3.0 — What's New\n\n### New Features\n\n**Palletizing — Generally Available** — NyankoOS now supports full pallet building with configurable layer patterns, mixed-SKU stacking, and overhang detection. Graduated from beta after 6 months of pilot deployments.\n\n**Role-Based Access Control** — Three built-in roles: Operator, Engineer, and Admin. Restrict dashboard and API access by role. Compatible with your existing SSO provider via SAML 2.0.\n\n**Multi-Camera Rig Support** — Configure up to 4 cameras per cell with automatic calibration using the new calibration wizard. Improves pick accuracy in large bin or conveyor-width scenarios.\n\n**Fanuc CRX Series Support** — Native integration with Fanuc CRX-10iA and CRX-25iA collaborative robots. No third-party middleware required.\n\n**Dark Mode** — The operator dashboard now supports dark mode. Toggle in user preferences.\n\n### Bug Fixes\n\n- Fixed palletizing pattern editor not rendering mixed-height layers\n- Fixed controller CPU spike to 100% during point cloud registration\n- Fixed task log export truncating beyond 10,000 records",
        "marketing_notes": "## NyankoOS v1.3: Palletizing Goes GA, Enterprise Access Controls Added\n\nNyankoOS 1.3 marks a major milestone: our palletizing module is now generally available, supported by 6 months of production deployments across 8 customer sites.\n\nCombined with new enterprise-grade RBAC and multi-camera support, NyankoOS 1.3 is our most enterprise-ready release to date.",
        "traceability": [
            {
                "short_hash": "1e2f3a4",
    ...
    },
    {
        ...
        "customer_notes": "## NyankoOS v1.5.0 — What's New\n\n### Performance Improvements\n\n**25% Faster Motion Planning** — Upgraded path planner reduces average planning time from 160ms to 120ms. Most noticeable on high-mix, high-frequency picking lines.\n\n**Parallel Vision Pipeline** — Object detection and depth estimation now run in parallel, cutting total vision processing time by 30%.\n\n### New Features\n\n**Manhattan Associates WMS Connector** — Native integration with Manhattan Active WM. Bidirectional task sync without custom middleware. Contact your account team to enable.\n\n**Webhook Task Events** — Subscribe to task completion, failure, and regrasp events via HTTP webhooks. Payload includes full task metadata and pick outcome.\n\n**Audit Log** — All operator and API actions are now logged with user identity and timestamp. Exportable from the dashboard.\n\n### Bug Fixes\n\n- Fixed false positive collision detection in reflective packaging environments\n- Fixed vision pipeline hang on zero-byte USB camera frames\n- Fixed throughput metrics not accounting for regrasp attempts",
    ...
    },
    {
        ...
        "readiness_score": 78,
        "recommendation": "HOLD",
        "internal_announcement": "## NyankoOS v2.0.0 — Major Release\n\nv2.0 is our biggest architectural change since launch. Async task engine, new v2 API, Python SDK 2.0, and multi-arm support.\n\n**⚠️ Breaking changes** — v1 API and SDK are deprecated (18-month support window). All customers need migration plans.\n\nReadiness: HOLD — we released with known compat CI failures on two v1 endpoints. Patch incoming.",
        "customer_notes": "## NyankoOS v2.0.0 — What's New\n\n### Architecture\n\n**Async Task Engine** — The core task execution engine has been rewritten with an async event loop. This enables NyankoOS to handle 10x more concurrent tasks without proportional CPU cost.\n\n**Multi-Arm Coordination** — Up to 4 robot arms can now share a workspace with automatic conflict resolution and priority-based handoff zones.\n\n### API & SDK\n\n**REST API v2** — Cleaner resource model, consistent pagination, and OpenAPI 3.1 spec. See the [migration guide](docs/api/v2-migration.md).\n\n**Python SDK 2.0** — New async-first SDK with full type annotations. Install: `pip install nyankoos-sdk>=2.0`.\n\n### Important: Breaking Changes\n\n**REST API v1 Deprecated** — `/api/v1/*` endpoints will continue to work for 18 months (until NyankoOS v2.6). All new integrations should use `/api/v2/*`.\n\n**SDK v1 Deprecated** — `RobotClient.connect_v1()` is deprecated. Migrate to `RobotClient.connect()`. Removal planned for v2.3.",
        "marketing_notes": "## NyankoOS 2.0: Built for Scale\n\nNyankoOS 2.0 is a ground-up redesign of the task execution engine, purpose-built for large-scale, multi-robot warehouse operations.\n\nThe new async architecture enables a single controller to coordinate up to 4 robot arms simultaneously — tripling the capacity of a v1.x installation without additional hardware.",
        "traceability": [
            {
                "short_hash": "9a0b1c2",
    ...
    },
    {
        ...
        "customer_notes": "## NyankoOS v2.1.0 — What's New\n\n### New Features\n\n**SAML 2.0 SSO** — Connect NyankoOS to your identity provider (Okta, Azure AD, Google Workspace) using SAML 2.0. All user sessions are brokered through your IdP.\n\n**OpenTelemetry Tracing** — Distributed traces for all controller operations are now exported in OTLP format. Compatible with Jaeger, Grafana Tempo, and any OpenTelemetry-compatible backend.\n\n**Cognex In-Sight 9000 Camera Support** — Native integration with Cognex In-Sight 9000 series smart cameras. No separate Cognex runtime license required.\n\n**Guided Setup Wizard** — New step-by-step wizard walks operators through first-time cell configuration: robot registration, camera calibration, gripper profile, and first task.\n\n### Performance\n\nPick accuracy across the fleet has improved to **99.2%** (up from 97.8% in v2.0), driven by improved depth sensor calibration and confidence-gating tuning.\n\n### Bug Fixes\n\n- Fixed NTP sync drift causing task timestamp skew on isolated networks\n- Fixed CSV import rejecting files with BOM encoding\n- Fixed arm oscillation at end-effector deceleration on payloads >10kg",
        "marketing_notes": "## NyankoOS v2.1: Enterprise Identity, Observability, and 99.2% Pick Accuracy\n\nNyankoOS 2.1 raises the bar on enterprise readiness with native SAML SSO, full OpenTelemetry observability, and a pick accuracy improvement that translates directly to fewer line stoppages.",
        "traceability": [
            {
                "short_hash": "5a6b7c8",
    ...
    },
    {
        ...
        "customer_notes": "## NyankoOS v2.2.0 — What's New\n\n### New Features\n\n**Variable Conveyor Speed** — Conveyor belt speed can now be dynamically controlled via PLC analog signal or API. NyankoOS automatically adjusts tracking parameters in real time.\n\n**KUKA iiQKA OS 1.2 Support** — NyankoOS now integrates natively with KUKA's iiQKA OS 1.2 motion interface. Supports all KUKA LBR iisy models.\n\n**Offline Task Queuing** — When cloud connectivity is lost, controllers now buffer up to 10,000 tasks locally and sync automatically on reconnection. No tasks are lost during network outages.\n\n**Predictive Maintenance Alerts** — Joint vibration data is now analyzed for anomalies. Alerts fire when vibration patterns suggest impending bearing wear, giving maintenance teams advance warning.\n\n### Performance\n\n- Conveyor tracking now stable at speeds up to 2.5 m/s (up from 2.0 m/s)\n- Point cloud registration CPU usage reduced by 35%\n\n### Bug Fixes\n\n- Fixed cycle count reset at midnight UTC crossover\n- Fixed task log export truncation at 10,000 records\n- Fixed URDF import failure with non-standard joint naming",
        "marketing_notes": "## NyankoOS v2.2: Resilient Operations and Predictive Maintenance\n\nNyankoOS 2.2 focuses on operational resilience — offline queuing ensures zero task loss during network outages, and predictive maintenance alerts help customers get ahead of hardware failures before they impact throughput.",
        "traceability": [
            {
                "short_hash": "9e0f1a2",
