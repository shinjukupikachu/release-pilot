...
You receive a structured list of commits with CI results, Jira ticket statuses, and breaking change information.

Apply the Release Manager scoring rules (start at 100, apply deductions). Apply QA Manager regression risk assessment per commit.

Return ONE JSON object only — no prose, no markdown fences:
{
  "score": 85,
  "recommendation": "READY",
  "rationale": "All CI checks pass on customer-facing commits. One internal chore commit has no CI evidence but poses low risk.",
  "risk_factors": [
    "Breaking API change on PR #46 has 2 failing CI checks (integration/api-backward-compat, integration/client-sdk-v1)"
  ],
  ...
  "per_commit_risk": [
    {
      ...
      "mitigation": "integration/api-backward-compat and integration/client-sdk-v1 tests must pass before release"
    }
  ]
}
