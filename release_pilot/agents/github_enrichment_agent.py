...
1. Call get_github_pr(commit_sha=<sha>) — extract: number, title (from title), url 
   (from html_url), author (from user.login)
...
If get_github_pr returns {"error": "not_found", ...}, 
 use: {"number": null, "title": null, "url": null, "author": null}
If get_check_runs returns zeros (not found), 
 use: {"total": 0, "passed": 0, "failed": 0, "failed_names": []}
